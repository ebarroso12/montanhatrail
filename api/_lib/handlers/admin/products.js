...
const { PRODUCT_COLUMNS, PRODUCT_FROM, toProduct } = require('../../catalog');
...
function readProduct(body) {
  ...
  if (!categoryId) throw new v.ValidationError('categoryId', 'Selecione a categoria.');
  ...
  if (req.method === 'POST') {
    const result = await db.query(
      `INSERT INTO products
       ...
       RETURNING id`,
      readProduct(parseBody(req))
    );
    res.status(201).json({ product: await findById(result.rows[0].id) });
    return;
  }
  ...
  if (req.method === 'PUT') {
    ...
    await db.query(
      `UPDATE products SET
       ...
       WHERE id = $15`,
      readProduct(parseBody(req)).concat(id)
    );
